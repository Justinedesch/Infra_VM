...
# Empty string for install variables $Path and $Port
$install_vars = ''
 
if ($Path) {
    $install_vars = $install_vars + " -Path $Path"
}
if ($Port) {
    $install_vars = $install_vars + " -Port $Port"
}
...
if ($install_vars) {
# If we have install vars
    Start-ChocolateyProcessAsAdmin "& `'$setupScript`' Install $install_vars"
} else {
# If we don't have install vars
